async function getCurrentWeatherByCity(city) {
    const data = await fetch(`http://api.weatherapi.com/v1/current.json?key=1e93a29ba7474169a2e173649250312&q=${city}&aqi=no`)
...
async function getCurrentForecastByCity(city) {
    const data = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=1e93a29ba7474169a2e173649250312&q=${city}&days=1&aqi=no&alerts=no`)
...
function currentForecastElement(iconSrc, time, temperature) {
...
function renderForecast(forecast) {
    ...
    forecast.forEach(forecastItem => {
        const forecastElement = currentForecastElement(forecastItem.condition.icon, forecastItem.time, forecastItem.temp_c)
